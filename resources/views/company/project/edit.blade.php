...
<div class="max-w-4xl mx-auto pb-10" x-data="projectForm({ 
    ...
    regency_id: {{ json_encode(old('regency_id', $project->regency_id) ?? '') }},
    district_id: {{ json_encode(old('district_id', $project->district_id) ?? '') }},
    village_id: {{ json_encode(old('village_id', $project->village_id) ?? '') }},
    address: {{ json_encode(old('address', $project->address) ?? '') }},
    regencies: [],
    districts: [],
    villages: [],
    ...
    async applyLocation(type) {
        ...
        if (loc) {
            ...
            const resReg = await fetch('/api/regencies/' + this.province_id);
            this.regencies = await resReg.json();
            
            this.regency_id = loc.regency_id;
            const resDist = await fetch('/api/districts/' + this.regency_id);
            this.districts = await resDist.json();
            
            this.district_id = loc.district_id;
            const resVil = await fetch('/api/villages/' + this.district_id);
            this.villages = await resVil.json();
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-6 md:p-8 border-b border-slate-100 bg-slate-50">
            ...
            <p class="text-slate-500 mt-1">
                {{ $isUMKM ? 'Perbarui profil produk, jasa, atau proposal KSO Anda.' : 'Perbarui kebutuhan pengadaan atau proyek Anda.' }}
            </p>
        </div>
        ...
        <form action="{{ route('projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8 space-y-8" @submit="validate">
            ...
            <div x-show="type !== ''" x-transition.opacity class="space-y-6 pt-6 border-t border-slate-100" style="display: none;">
                ...
                <div x-data="{ imageUrl: '{{ $project->image ? Storage::url($project->image) : '' }}' }">
                    ...
                    <div class="mb-4" x-show="imageUrl" style="display: none;">
                        <img :src="imageUrl" alt="Banner Proyek" class="h-40 rounded-xl object-cover border border-slate-200">
                    </div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Gunakan foto beresolusi baik agar terlihat menarik di halaman detail. Maks. 5MB. Biarkan kosong jika tidak ingin mengubah gambar.</p>
