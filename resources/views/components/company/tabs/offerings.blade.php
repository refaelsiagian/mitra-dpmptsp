...
<div x-data="{ activeProjectTab: 'aktif' }" wire:key="offerings-component-{{ $company->id }}">
    <div class="flex justify-between items-center mb-6" x-data="{ openDropdown: false }" wire:key="offerings-header-{{ $company->id }}">
    ...
    <div x-show="activeProjectTab === 'aktif'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="" wire:key="offerings-aktif-{{ $company->id }}">
        @if($activeCount === 0)
        ...
        @endphp
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group hover:border-blue-300 transition-colors mb-6 last:mb-0">
    ...
    <div x-show="activeProjectTab === 'selesai'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;" class="" wire:key="offerings-selesai-{{ $company->id }}">
        @if($closedCount === 0)
            <div class="text-center p-10 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 mt-2">
